 ...
 * Histograma de valores inteiros - versao PARALELA (OpenMP)
 *
 * Uso (igual ao serial):
 *   ./histograma_paralelo <tamanho> <arq>
 * Saida: histograma.out
 *
 * Problema de concorrencia: a operacao hist[bin]++ e' uma REGIAO CRITICA, pois
 * varias threads podem incrementar o MESMO 'bin' ao mesmo tempo (condicao de
 * corrida / dependencia de dados entre iteracoes que caem no mesmo valor).
 *
 * Solucao adotada: REDUCAO DE VETOR  -> reduction(+:hist[:faixa]).
 * O OpenMP cria uma copia PRIVADA do histograma por thread (sem disputa durante
 * a contagem) e, ao final, soma todas as copias na global (essa combinacao final
 * e' a regiao critica, gerenciada automaticamente pela clausula reduction).
 * Isso escala muito melhor do que usar '#pragma omp atomic' em cada incremento.
 */
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
int main(int argc, char *argv[]) {
    ...
    fprintf(stderr, "[paralelo] n=%d  faixa=%d  threads=%d  tempo=%.6f s\n",
            n, faixa, omp_get_max_threads(), t1 - t0);
